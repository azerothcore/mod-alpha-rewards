...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "view", SEC_PLAYER, true, &handleViewCommand, ""},
    ...
    static bool handleViewCommand(ChatHandler* handler, char const* args)
    {
        ...
        {
            handler->PSendSysMessage("Player: %s Currently has %u GamePoints", target->GetName().c_str(), target->CustomData.GetDefault< AlphaRewardData>("RewardPointsMap")->RewardPointsMap);
        }
        else if (*args && IsGM && !target) // if no target selected but if arguments
        {
            ...
            if (CheckPlayer)
            {
                handler->PSendSysMessage("Player: %s currently has %u Gamepoints", CheckPlayer->GetName().c_str(), CheckPlayer->CustomData.GetDefault<AlphaRewardData>("RewardPointsMap")->RewardPointsMap);
            }
            else
            {
                std::string str = args;
    ...
    static bool handleAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            uint32 amount = atoi(args);
                // target
                target->CustomData.GetDefault<AlphaRewardData>("RewardPointsMap")->RewardPointsMap += amount;
        }
        else
        {
            ...
            auto* session = sWorld->FindSession(AccountMgr::GetId(accountName));
            ...
            if (session)
            {
                ...
                if (player)
                {
                    player->CustomData.GetDefault<AlphaRewardData>("RewardsPointMap")->RewardPointsMap += amount;
                    handler->PSendSysMessage("You have given account %s %u Gamepoints", accountName, amount);
                }
            }
            else
            {
                QueryResult result = LoginDatabase.PQuery("SELECT `id`, `username` FROM `account` WHERE `username` = '%s'", accountName);

                if (!result)
                {
                    handler->PSendSysMessage("The account name '%s' does not exist", accountName);
                    return false;
                }
